...
# changed by Hongjing in 2021/07/12
# aiming at avoiding the corruption of downloading
import socket
...
def dlControl(count, blockSize, totalSize):
    ...
    text2 = "\r%s %i%% |%s%s| %i/%iMB %.2f MB/s\r" % (
        "Downloading ", percent, "█"*block, "░"*(barLength-block), csize, tsize, tRate)
    sys.stdout.write(text2)
    sys.stdout.flush()

    if (tRate < 0.001) and (passedTime > 1000):
...
def dl_cmip6(durl, dir_path):
    ...
    if (not os.path.exists(dir_path / durl.split('/')[len(durl.split('/'))-1])):
        print("\n\n"+durl.split('/')
              [len(durl.split('/'))-1]+" is available!\n")
        try:
            urllib.request.urlretrieve(durl, durl.split(
                '/')[len(durl.split('/'))-1], reporthook=dlControl)
        except socket.timeout:
            # if timeout then removing this file and retrying
            count = 1
            print("Retrying...Removing file . . .\n")
            if (not os.path.exists(dir_path / durl.split('/')[len(durl.split('/'))-1])):
                print("Failed to linking the server...file not exist!\n")
            else:
                os.remove(durl.split('/')[len(durl.split('/'))-1])
            startTime = time.time()
            while count <= 5:
                try:
                    urllib.request.urlretrieve(durl, durl.split(
        ...
        print("\n"+durl.split('/')
              [len(durl.split('/'))-1]+" already exists!\n")
...
def DownloadCmip6(**kwargs):
    ...
    if (links == []):
        ...
        print('\n'+color.UNDERLINE+color.BOLD+'TIPS 1:'+color.END +
              ' Use the check (-c) argument to check your inputs.'+color.END)
    ...
    for url in links:
        ...
        try:
            ...
            manual = manual+1
            print("\n"+color.RED +
                  "<<401 Unauthorized: restricted access!!>>"+color.END+"\n")
            print(color.UNDERLINE+"From ESGF:"+color.END +
                  " Before you can download this data, you have to join a data access control group \nsince acknowledgement of a policy is a condition for this data download.")
