    ...
    def get_links(self, manual):
        if manual == 0:
            links = _Construct_urls(
                self._var, self._mod, self._realm, self._exp, self._freq)._get_wget(0)
        else:
            links = _Construct_urls(
                self._var, self._mod, self._realm, self._exp, self._freq)._get_wget(1)
        return links

    def get_info(self):
        info = _extract_info(self._var, self._mod, self._realm, self._exp,
                             self._freq, self.n_files, self.rlzn, self.year, self.links)._get_info()
        return info

    def get_url(self):
        url = _Construct_urls(self._var, self._mod,
                              self._realm, self._exp, self._freq)._get_url()
        return url
