...
class _realizations(object):
    ...
    def _all_realizations(self):
        ...
        for i in range(len(self.links)):
            try:
                er[i] = int(self.links[i].split(
                    '/')[len(self.links[i].split('/'))-1].split('_r')[1][0:2])
            except IndexError:
                try:
                    er[i] = int(self.links[i].split(
                        '/')[len(self.links[i].split('/'))-1].split('-r')[1][0:2])
                except:
                    er[i] = int(self.links[i].split(
                        '/')[len(self.links[i].split('/'))-1].split('-r')[1][0])
            except:
                try:
                    er[i] = int(self.links[i].split(
                        '/')[len(self.links[i].split('/'))-1].split('_r')[1][0])
                except ValueError:
                    er[i] = int(self.links[i].split(
                        '/')[len(self.links[i].split('/'))-1].split('_r')[2][0])
...
class _extract_info:
    ...
    def _get_info(self):
        if self.links == None:
            links = _Construct_urls(
                self.var, self.mod, self.realm, self.exp, self.freq)._get_wget(0)
        else:
            links = self.links
...
def _mod_help():
    print("\n"+color.PURPLE +
          "                <<You are using the CMIP6DB module now>>"+color.END)
    print("\nUse 'show' as the -m/-e/-f/-v/-r argument option instead of the names\n")
    print(color.BOLD+color.UNDERLINE+"Example:"+color.END +
          " acccmip -o M -m show >>> will generate a list of all currently available models in the CMIP6 database.")
...
def _get_rlzn_links(r, all_rlzn, links):
    ...
    for rlzn in r:
        if rlzn in str(all_rlzn):
            for url in links:
                # print(url.split('/')
                #   [len(url.split('/'))-1].split('_r')[1].split('i')[0])
                try:
                    try:
                        # changed by Hongjing in 2021/07/14
                        # to fix the bug that when set the realization to 10 but actually download the r10,r101,r102...problem
                        if (int(rlzn) == int(url.split('/')[len(url.split('/'))-1].split('-r')[1].split('i')[0])):
                    ...
                    try:
                        # changed by Hongjing in 2021/07/14
                        # to fix the bug that when set the realization to 10 but actually download the r10,r101,r102...problem
                        if (int(rlzn) == int(url.split('/')[len(url.split('/'))-1].split('_r')[1].split('i')[0])):
